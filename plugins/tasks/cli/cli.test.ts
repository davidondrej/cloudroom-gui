...
describe("bb tasks CLI", () => {
  ...
  it("self-attaches through ROOM_THREAD_ID and lists the live thread status", async () => {
    const { bb, harness } = createFakePluginHost({
      pluginId: "tasks",
      sdk: {
        threads: {
    ...
    const previousThreadId = process.env.ROOM_THREAD_ID;
    process.env.ROOM_THREAD_ID = "thr_cli_self";
    try {
      expect(
        JSON.parse(stdout(await harness.runCli(["attach", "ATT-1", "--json"]))),
      ).toMatchObject({ task: { key: "ATT-1" }, threadId: "thr_cli_self" });
    } finally {
      if (previousThreadId === undefined) delete process.env.ROOM_THREAD_ID;
      else process.env.ROOM_THREAD_ID = previousThreadId;
    }
