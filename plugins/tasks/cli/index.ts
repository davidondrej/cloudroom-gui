...
function projectTable(
  ...
  return table(
    ["PREFIX", "NAME", "FOLDER", "ROOM PROJECT", "ID"],
...
function resolveInvokingThreadId(
  ...
  const threadId =
    option(args, "thread") ?? process.env.ROOM_THREAD_ID ?? ctx.threadId;
  if (!threadId) {
    throw new CliError("missing --thread and ROOM_THREAD_ID is not set");
  }
  return threadId;
}
