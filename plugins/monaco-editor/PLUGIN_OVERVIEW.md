Edit a file in Room instead of only reading it. It applies to every place where Room opens a file: chat links, the file search, and `room thread open`. The editor is Monaco, the editor from VS Code.
...
- Colors that follow your Room theme, including light and dark switches and custom palettes.

## How it works

The plugin claims common code, configuration, and text extensions. Binary files such as images and PDFs stay with Room's own preview. Files larger than 8 MB open in the read-only preview.
