Use one Cloudroom workspace with the coding agents you already run on your machine. This plugin connects Cloudroom to agents that speak the Agent Client Protocol (ACP). Each agent appears as a provider in the thread composer.
...
- Skills from the agent's own skill directories, listed next to Cloudroom skills.
