...
description: "Configure or troubleshoot Room-specific Claude Code provider settings and session behavior."
---

# Claude Code provider

Read settings with `room plugin config provider-claude-code`; change a declared key
with `room plugin config provider-claude-code set <key> <value>`.
...
- Structured plan, message editing, and compaction are supported through the
  corresponding `room thread` commands. Unlisted model IDs are accepted by the
