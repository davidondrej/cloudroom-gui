Start a thread, pick Claude Code, and let it work in your repository from Room. The plugin drives the Claude Code CLI on the host machine. It streams the agent's work into the Room timeline.
...
- `Disable provider subagents`: hide the native Task tool so the agent delegates through Room. On by default.
...
- Sign in with `claude` on that machine. Room reads the sign-in state to show account and plan.
