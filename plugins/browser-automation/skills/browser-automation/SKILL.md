...
description: Use the Browser Automation Cloudroom plugin to inspect and automate persistent browser pages in an explicit desktop or local headless session. Use for browser navigation, snapshots, clicking, forms, and verification screenshots.
---

Use `cloudroom browser-automation`. Open one session, retain its session ID, then inspect,
...
Resolve the explicit instance with `cloudroom browser instances --host <host-id> --json`
...
```sh
cloudroom browser-automation open --backend local --headless --machine <host-id> --json
cloudroom browser-automation open --backend desktop --machine <host-id> --desktop <instance-id> --json
```

Run scripts:

```sh
cloudroom browser-automation run <session-id> --script 'const p = await browser.getPage("main"); await p.goto("https://example.com"); await p.snapshot()' --json
cloudroom browser-automation run <session-id> --script 'const p = await browser.getPage("main"); await p.click("ref/e6"); await p.snapshot()' --json
cloudroom browser-automation screenshot <session-id> --page main --json
```
...
```sh
cloudroom file read '<image-path>' --host '<host-id>' --json | node -e '
