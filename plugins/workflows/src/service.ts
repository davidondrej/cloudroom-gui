...
export function formatWorkflowNotification(
  ...
  const prefix = `[room workflow finished · ${run.id}]\n\nRun ${run.id} (${run.name}) ${run.status}.\n`;
  const suffix = `\nRun \`room workflows status ${run.id}\` for authoritative details.`;
  const detail =
  ...
  return utf8Prefix(
    `[room workflow ${run.id}] ${run.status} — run room workflows status ${run.id}`,
    maximumBytes,
  );
}
...
export function createWorkflowService(
  ...
  function childPrompt(
    ...
    const header = `[room workflow ${run.name} · run ${run.id}]`;
      ...
      instructions:
        call.status !== "running"
          ...
          : options.outputSchema === null
            ? null
            : `You are a Room workflow worker. Submit your final value with bb_workflow_result. Required schema: ${JSON.stringify(options.outputSchema)}`,
    };
  }
