...
const runInputSchema = z
  .object({
    ...
    resumeRunId: z
      ...
      .nullable()
      .describe(
        "Run ID of a prior Room workflow to resume from. Calls in the causally safe, longest unchanged prefix return cached results; the first edited, new, or concurrent call and everything after it run live. The prior run must be terminal and from the same project and environment.",
      )
      .default(null),
  })
  .strict();
  ...
  bb.agents.configure((context) => {
    ...
    if (context.origin.pluginId === bb.pluginId) {
      return {
        ...
        skills: [],
        instructions:
          "You are starting as a Room workflow worker. Follow the workflow prompt. Your final text IS the return value, not a human-facing message. If the prompt requests structured output, call bb_workflow_result exactly once at the end of your response.",
      };
    }
    return {
      ...
      skills: ["workflows"],
      instructions:
        "When bb_workflow_run succeeds, copy its previewDirective into your response exactly once as a standalone line. Do not wrap it in backticks or a code fence, and do not invent or edit the run ID. The directive renders live workflow progress in Room chat. `room workflows status <run-id>` returns a compact summary. For detailed history, redirect `room workflows history <run-id> --cursor <call-index> --limit <1-100>` into a file under `$ROOM_THREAD_STORAGE`, then inspect that JSONL file with normal filesystem tools. Use each page record's `nextCursor` to continue.",
    };
  });
