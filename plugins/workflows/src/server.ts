...
const runInputSchema = z
  .object({
    ...
    resumeRunId: z
      ...
      .nullable()
      .describe(
        "Run ID of a prior Cloudroom workflow to resume from. Calls in the causally safe, longest unchanged prefix return cached results; the first edited, new, or concurrent call and everything after it run live. The prior run must be terminal and from the same project and environment.",
      )
      .default(null),
  })
  .strict();
  ...
  bb.agents.configure((context) => {
    ...
    if (context.origin.pluginId === bb.pluginId) {
      return {
        ...
        skills: [],
        instructions:
          "You are starting as a Cloudroom workflow worker. Follow the workflow prompt. Your final text IS the return value, not a human-facing message. If the prompt requests structured output, call bb_workflow_result exactly once at the end of your response.",
      };
    }
    return {
