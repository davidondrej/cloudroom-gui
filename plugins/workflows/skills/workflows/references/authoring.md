...
- `agent(prompt: string, opts?)`: spawn a Cloudroom worker. Without `schema`, returns
...
Workers spawned by `agent()` are normal Cloudroom threads and retain the tools and
workspace access allowed by their Cloudroom permission mode. The QuickJS script itself
...
Cloudroom validates the tuple against the live provider/model catalog immediately
...
Native `label` is an alias for Cloudroom's existing `title`, and native `schema` is an
alias for Cloudroom's existing `outputSchema`. Either spelling remains supported.
...
the structured output. Cloudroom validates the value with Ajv. The initial invalid
