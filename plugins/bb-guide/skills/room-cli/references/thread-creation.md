...
- Use `room thread spawn --project <project-id> --prompt "..."` to create another
  ...
  without a remembered model, room resolves the selected provider and its reported
...
- List machine providers with `room machine providers`. Create a
  new provider machine with
  `room thread spawn --new-machine <provider-id> --environment-provider <id>`.
...
- Handoff can target any model, including one from the source provider. In
  ...
  keeps handoff active. Use `room thread spawn --provider PROVIDER --model MODEL
...
- Use `room thread fork <source-thread-id>` to clone a provider session. The
...
- Pass `--visibility hidden` for background/plugin workers that should remain
  ...
  attention. `room thread list` excludes them by
  ...
  existing thread with `room thread update <id> --visibility visible|hidden`.
- Stop a finished hidden worker with `room thread stop <id>` to release its agent
...
- Add remote execution machines from Settings → Machines. Its one-line
  ...
  both the daemon protocol and agent-launched `room` CLI to traverse the account
  ...
  only on a 404. It installs under the enrollment's room data directory, without
  ...
  transient backoff, use `room machine retry-update <id-or-name>`. Remove
  `--auto-update` from the service definition and reload it to opt out.
- Run `room machine list` to see machine names, IDs, type, connection status, and
  ...
  (alias `--host`) on `room thread spawn` to run in a personal or unmanaged
...
- Each machine carries a permission limit (`maxPermissionMode`, default
  ...
  `room machine list --json` or `room machine show` and ask the user to change it
  in the app.
- `room machine providers`, `show`, `join-code`, `rename`, `retry-update`,
...
- Use `room project create --name <name> --root <path> --machine <id-or-name>`
...
- `room project list` preserves the ordinary-project-only default. Pass
  `--include-personal` when the singleton personal project must be discoverable.
- Use `room project source add <project-id> --machine <id-or-name> --path <path>`
...
- `room project paths|files|content|commands` accept `--machine <id-or-name>`
  ...
  `room project content --json` returns UTF-8 text or base64 binary content with
  ...
  workspaces use filesystem listing. `room file list|paths` can inspect ignored
  files, subject to their exclusion options.
- Use `room project attachment upload <project-id> --client-file <path>` when the
  bytes live on the CLI machine, including when the CLI and room server are on
  ...
  are rejected (convert them to JPEG or PNG first). `room project attachment
download <project-id> <attachment-path> --client-file <path>` writes existing
  ...
  per-file remove API.
- `room project history|reorder` exposes project prompt recall and sidebar order.
- Use `room project show|update|delete` for one project. Use `room project source
update|delete` for one source. Use `room project branches` for branch data.
- Direct environment inspection accepts any environment ID: use `room environment
...
- `room environment pull-request ready|draft|merge` manages pull-request state;
  `room environment archive-threads` bulk-archives an environment's threads.
- Use `room environment show|update|commit` for environment metadata
...
- If provider or model choice matters, inspect options with `room provider list`
  and `room provider models <provider-id>`. Both accept `--machine <id-or-name>`
...
- Top-level `sharedSkillRoots` uses the same relative `user` and `project`
  ...
  so one physical skill collection can support room and standalone provider CLIs.
...
Creation is durable. `--no-wait` returns the host ID; `room machine show
<host-id>` inspects it and `room machine remove <host-id>` cancels it. SIGINT
stops following and exits with status 130 while creation continues.

`room machine show <id-or-name> --json` includes `providerDetails` inventory and
...
installation location; the installer's BB_DATA_DIR is also an assertion, never permission to
...
Use `room machine enroll --bootstrap-file <path>` or `--bootstrap-env <NAME>` on a machine that already has the CLI. Core prepares the versioned bundle; transport it through a private file or environment/stdin, never command arguments, logs, resource JSON, or a transcript. Enrollment refuses a different existing host/server identity and succeeds without another exchange when the same identity is already enrolled. The installer accepts `--bootstrap-env <NAME>` and invokes this command after installing bb. Machine state defaults to `~/.bb-machines/<server-host>`; an explicit `ROOM_DATA_DIR` must be isolated from the default BB instance. For remote non-login commands, discover `room` on PATH and fall back to `~/.local/bin/room`.
...
Personal file access: `room project paths|files|content proj_personal` requires
