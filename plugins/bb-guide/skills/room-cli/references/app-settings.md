...
- `room settings general <key> <value>` accepts any key listed under
  `generalSettings` in `room settings show`. Boolean preferences take `true`,
...
- `room settings ui set <key> <value> [--json]` takes a plain string for enum
  and provider keys and JSON for lists or `null`; it reads the current
  revision, writes with it, and retries once on a conflict.
- `room settings ui reset <key> [--json]` writes the default and advances the
  revision.

## Keyboard shortcuts

- `showKeyboardHints` defaults to true. Set it with
  `room settings keyboard hints <true|false|on|off>` to control whether
...
- `showDiagnosticEvents` defaults to false in all builds. Set it with
  `room settings general showDiagnosticEvents <true|false|on|off>`.
...
- `steerActiveThreadOnEnter` defaults to false (Queue). Existing saved
  ...
  `room settings general steerActiveThreadOnEnter <true|false|on|off>`.
...
- `streamerMode` defaults to false. Set it with
  `room settings general streamerMode <true|false|on|off>`.
- When enabled, every `customModels` entry from `~/.bb/config.json` is hidden
  in all model lists: the pickers, `room provider models`, and
...
- `managedBranchPrefix` defaults to `bb/`. Set it with
  `room settings general managedBranchPrefix <prefix>`.
- room puts the prefix in front of every branch name it creates for a managed
...
- room rejects a prefix that cannot start a valid git branch name, such as one
  with a space or a leading `-`. The maximum length is 64 characters.
- The new prefix applies to branches room creates after the change. It does not
...
- When a turn finishes, room can collapse its work into one `Worked for` row
...
- `room settings completed-turns [--json]` lists every provider with its current
  display and whether it comes from your setting or the provider default.
- `room settings completed-turns <provider-id> <collapse|flat|default>` sets the
...
- The overrides are stored in `providerCompletedTurnDisplay`, a map of provider
  ...
  outline, and `room thread log`.

## Message edits

- Eligible accepted root user messages can be edited without enabling an
  experiment. Use `room thread edit-message` or the message editor in the app.
...
- The `mobileApp` experiment defaults to false while the room mobile app is in
  early access.
- Enable it with `room settings experiment mobileApp true`. It shows the
  **Add mobile device** card under Settings → Remote access.

## Changelog preview

- The `changelogPreview` experiment defaults to false.
- Enable it with `room settings experiment changelogPreview true` to show the
...
- Enable it with `room settings experiment sidebarProgressiveDisclosure true`.
...
- Enable it with `room settings experiment timelineWindowing true`.
...
- Enable it with `room settings experiment multiMachinePicker true`.
...
Machine access: `room settings general machineServerUrl https://bb.example.com`
sets the server URL reachable by machines. Set `null` to use BB_EXTERNAL_URL.
`room settings general defaultMachineAccess direct` selects direct access;
...
reports setup required. `room settings show --json` includes serverAccess with the
...
Use `room settings ui reset sidebar.hiddenFooterItems` to show everything again.

Disable anonymous usage telemetry with `room settings general telemetryEnabled false`
