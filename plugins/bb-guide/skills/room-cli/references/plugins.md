...
- A room plugin is a TypeScript package running inside the room server, extending
...
- **BB plugin catalog** (store under `/api/v1/plugin-catalog`):
  ...
  - The store lists the **BB Community marketplace** catalog: a manifest
    ...
    plugin code, and a failed refresh keeps the last catalog room validated.
  - `room plugin search <query> [--json]` — search the catalog by id,
...
- **Third-party marketplaces** (routes under `/api/v1/marketplaces`):
  ...
  - `room marketplace refresh [name] [--json]` — re-read one catalog or every
    one of them. Discovery metadata and icons only. A failed refresh keeps the
    last catalog room validated and exits non-zero.
  - `room marketplace remove <name> [--json]` — forget a marketplace. Its
    ...
    resolution, so `room plugin outdated`/`update` keep working from the
    recorded source.
  - Install a specific marketplace's entry with
    `room plugin install <entry-id>@<marketplace>`. A bare entry id resolves
  ...
  - Install a bundled plugin by its bare name or with
    `<entry-id>@bb-official`. room copies it from the app bundle.
- Commands:
  - `room plugin install <src>` — `<entry-id>@<marketplace>`, an HTTP(S) Git
    ...
    on its own. room records the selected tag and its commit and refuses to
    ...
    refused — use `room plugin update`. Installing a local path for an id that
    ...
    Managed git/npm installs refuse `engines.room` /
  ...
  - `room plugin outdated` — check installed plugins for compatible updates
  ...
  - `room plugin update <id>` / `room plugin update --all` — apply compatible
    ...
    non-TTY refuses without it). Use `room plugin outdated` to preview available
    updates. Changing a pinned git:/npm: source requires `room plugin remove`
    ...
    place and `room plugin reload <id>`, or `room plugin install path:<new dir>`
    to move it; both keep its configuration.
  - `room plugin list` — status, background services, schedules, handler timings,
    and each plugin's contributed `room` command.
  - `room plugin source <id> [--json]` — requested and resolved source, the
  ...
  - `room plugin enable|disable <id>`, `room plugin reload [id]` (exits 1 when a
    ...
    `room plugin remove <id>` (deletes the plugin's settings, secrets, and
    schedules; managed git/npm files are deleted, and local path sources stay
    on disk).
  - `room plugin config <id> [set <key> <value> | unset <key>]` — declared
    settings; boolean and number arguments are converted to their declared
    types. Reload the plugin after configuring (`room plugin reload <id>`).
  - `room plugin token <id>` — print a short-lived bearer token for that plugin.
  - List, reload, enable, disable, config, and remove support `--json`.
  - `room plugin logs <id> [-n N] [-f]` — the plugin's `bb.log` output.
  ...
  - `room plugin new <name>` — scaffold a todo-list plugin (`server.ts`,
    ...
    rather than reporting success; `room plugin build [path]` —
  ...
  - `room plugin types [path]` — sync the plugin's `@get-bb/plugin-sdk` surface
    ...
    mismatch (for CI). `room plugin build` and `room plugin dev` refresh vendored
  ...
  - `room plugin dev [path]` — watch loop for an installed plugin (default:
    cwd): on every change it rebuilds the frontend bundle (when `bb.app` is
    declared; unminified, unlike `room plugin build`) and reloads the plugin;
...
- **Writing a plugin?** Use the `bb-plugin-authoring` skill — the complete
  ...
  and gotchas. `room guide plugins` has the short walkthrough.
