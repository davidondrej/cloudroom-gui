...
- Add `--plan` to `room thread spawn` or `room thread tell` to send the prompt as
  ...
  plan with `room thread interactions`; `room thread cancel-plan` leaves Plan mode
  early. The SDK equivalent is `input: [createBuiltinPlanCommandTextInput(text)]`
  (exported by `@bb/sdk`) on `threads.spawn` / `threads.send`.
- Use `room thread edit-message <thread-id> --message "..."` to replace and rerun
  ...
  policy.
- Add `--send-at <when>` to `room thread spawn` or `room thread tell` to schedule
...
- `room thread queue list` shows a Sender for agent threads and system notices.
...
- Inspect and act on queued dispatches with `room thread queue list [<thread-id>]
[--wait-holder plugin:<plugin-id>]`, `room thread queue send <thread-id>
<message-id>` (send it now, bypassing every plugin wait and its schedule), and
  `room thread queue delete <thread-id> <message-id>` (discard it). Omitting the
...
- `room thread queue send <thread-id> <message-id> --mode steer` re-attempts the
...
- Use `room thread count` when you need how many threads there are, never a list
  plus a row count: the count is a database aggregate, while `room thread list`
...
- `room thread tell` steers Local threads by default and queues Cloud messages.
  ...
  Example: `room thread tell <thread-id> "Stop and use approach B" --mode steer`.
...
- If the target thread is awaiting user interaction (an open question or
  approval), `room thread tell` cannot interrupt it. The message joins the
  ...
  not a failure, so do not resend. For a hard stop use `room thread stop
...
- Use `room thread search <query> [--limit <1-50>]` for sidebar search. Use
  `history`, `read|unread`, and `section` for organization and recall. The
  `room thread queue` group contains the queued-message operations. Queue updates
...
- Use `room thread show <thread-id>` for status, parent, environment, pull request
  status, and result.
- Use `room thread show <thread-id> --git-diff` to review file changes.
- Use `room thread log <thread-id>` to inspect the conversation. The default
...
- Use `room thread output <thread-id>` to read the latest final output, or
  `room thread output --self` for the current thread.

For review or fix pipelines, get the environment ID from
`room thread show <thread-id> --json`, then spawn the follow-up with
...
- A file path is optional when a thread ID is explicit:
  `room thread open <thread-id> [path] [--split <placement>]`.
...
- Use `room file read|write|list|paths|mkdir|move|remove` for SDK-equivalent host
...
- Use `room voice transcribe <file> [--type <mime>] [--prompt <text>]` without the
...
- `room terminal start` is an alias for create. `room terminal stop` is an alias
  for close.
- Use `room terminal show`, `attach`, and `resize` for session inspection,
...
- All existing-session operations need only the terminal ID. Use
  `room terminal wait <terminal-id> --contains "Local:" --timeout 120` to wait
  for readiness from new output. Pass `--from-start` only when matching existing
  scrollback is intentional.
- Use `room terminal output <terminal-id> --json` to read bounded output, then
  continue with `--since-seq <nextSeq>` when polling. Use
  `room terminal send <terminal-id> --text "..." --enter` for interactive input,
  `room terminal rename <terminal-id> <title>` to rename, and
  `room terminal close <terminal-id>` when the process is no longer needed.
- `room terminal restart <terminal-id>` replaces the session with a shell in the
  same scope, size, and title. It does not replay the original launch command.

Clearing a thread's parent with `room thread update --clear-parent-thread` inherits the former parent's section unless the update explicitly supplies a section. Children released by environment archiving also inherit their former parent's section.
