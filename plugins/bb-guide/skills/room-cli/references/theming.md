# room theme authoring reference

How to write a good room app theme: the design model, the full design-token
...
The folder name is the theme id. Run `room theme dir` to print the exact directory
...
and re-run `room theme set <name>` (or just re-select it) to re-apply.
...
1. `room theme dir` — print the custom-theme directory (e.g. `~/.bb/theme`).
2. Write your stylesheet to `<that-dir>/<name>/theme.css`. Start the name with
   ...
   hyphens.
3. `room theme set <name>` — activate it. To edit later, change the file in place
   and re-run `room theme set <name>`.
...
There is no separate code-theme setting. `room theme show` prints the resolved
