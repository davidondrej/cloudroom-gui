...
- For failed threads, inspect `room thread show <id> --json` and
  `room thread log <id>` before deciding whether to retry, clarify, or update the
  user.
- Use `room thread retry <thread-id>` to re-send a failed turn's original message
  ...
  queue the retry on the clock (same `<when>` grammar as `room thread tell
...
- Use `room thread stop <id>` when a thread is stuck or no longer needed.
- `room thread stop <id>` also releases an idle or stuck agent runtime. The
