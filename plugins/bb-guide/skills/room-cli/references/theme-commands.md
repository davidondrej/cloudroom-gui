...
- `room theme` controls the **app-wide color palette** — a set of CSS-variable
...
- Commands:
  - `room theme list` — built-in and custom themes and which palette is active.
  - `room theme dir` — print the absolute custom-theme directory (where to create
    `<name>/theme.css`). Use this instead of guessing the path.
  - `room theme set <id> [--favicon-color <color>]` — activate a built-in
  ...
  - `room theme favicon set <color>` — set the favicon color while preserving the
    ...
    `blue`, `purple`, `pink`.
  - `room theme favicon reset` — reset the favicon color to `default` while
...
3. Activate it: `room theme set <name>`. Changes apply live to every open window.
