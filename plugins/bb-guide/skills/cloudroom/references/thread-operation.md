...
`cloudroom thread tell` automatically queues Cloud messages. Local messages steer by default. Cloud supports follow-ups, inspection, stop, title changes, pinning, and archive/restore. Stop pauses the queue; `cloudroom thread queue send THREAD FIRST_MESSAGE_ID` resumes in order. Inspect `cloudroom cloud status --json`: steering, attachments, compaction, message editing (`rewind`), and queue editing/cancellation depend on the connected core and harness. Forks/children, scheduling, and session-model changes remain unavailable. Confirm actual outputs, not just advertised capabilities.
...
- Use `cloudroom thread wait <thread-id>` to wait for a status or event. It defaults
...
- Use `cloudroom thread tell <thread-id> "..."` when requirements change, a blocker
  needs clarification, or follow-up work is needed.
- Add `--plan` to `cloudroom thread spawn` or `cloudroom thread tell` to send the prompt as
  ...
  plan with `cloudroom thread interactions`; `cloudroom thread cancel-plan` leaves Plan mode
  early. The SDK equivalent is `input: [createBuiltinPlanCommandTextInput(text)]`
  (exported by `@bb/sdk`) on `threads.spawn` / `threads.send`.
- Use `cloudroom thread edit-message <thread-id> --message "..."` to replace and rerun
  ...
  carries its `ROOM_THREAD_ID` so the replacement runs under agent permission
  policy.
- Add `--send-at <when>` to `cloudroom thread spawn` or `cloudroom thread tell` to schedule
...
- `cloudroom thread queue list` shows a Sender for agent threads and system notices.
...
<message-id>` (send it now, bypassing every plugin wait and its schedule), and
  `cloudroom thread queue delete <thread-id> <message-id>` (discard it). Omitting the
...
- `cloudroom thread queue send <thread-id> <message-id> --mode steer` re-attempts the
...
- Use `cloudroom thread count` when you need how many threads there are, never a list
  plus a row count: the count is a database aggregate, while `cloudroom thread list`
...
- `cloudroom thread tell` steers Local threads by default and queues Cloud messages.
  For Local threads, use `--mode queue` when the message is non-urgent and the
  ...
  Example: `cloudroom thread tell <thread-id> "Stop and use approach B" --mode steer`.
...
- If the target thread is awaiting user interaction (an open question or
  approval), `cloudroom thread tell` cannot interrupt it. The message joins the
...
- Use `cloudroom thread search <query> [--limit <1-50>]` for sidebar search. Use
  `history`, `read|unread`, and `section` for organization and recall. The
  `cloudroom thread queue` group contains the queued-message operations. Queue updates
...
- Use `cloudroom thread show <thread-id>` for status, parent, environment, pull request
  status, and result.
- Use `cloudroom thread show <thread-id> --git-diff` to review file changes.
- Use `cloudroom thread log <thread-id>` to inspect the conversation. The default
...
- Use `cloudroom thread output <thread-id>` to read the latest final output, or
  `cloudroom thread output --self` for the current thread.

For review or fix pipelines, get the environment ID from
`cloudroom thread show <thread-id> --json`, then spawn the follow-up with
`--environment <environment-id>` so it sees the same files.

## Opening Threads And Files In The App

- Reference a Cloudroom thread in chat as `@thread:thr_abc123`, substituting its
  actual ID. Cloudroom renders the mention with the correct project-aware link; do not
  construct `/threads/...` or `/projects/.../threads/...` URLs manually.
- Use `cloudroom thread open <path>` inside a Cloudroom thread to open a Markdown, HTML, or
  other workspace file for the user in the Cloudroom IDE's thread panel.
- Use `cloudroom thread open <thread-id> --split right|down|left|top|replace` to open
...
- A file path is optional when a thread ID is explicit:
  `cloudroom thread open <thread-id> [path] [--split <placement>]`.
...
- Use `cloudroom file read|write|list|paths|mkdir|move|remove` for SDK-equivalent host
...
- Use `cloudroom voice transcribe <file> [--type <mime>] [--prompt <text>]` without the
  app composer. The MIME type defaults to `audio/webm`.

## Long-Running Commands

- Use `cloudroom terminal ...` for long-running commands the user may need to inspect
  or stop later: dev servers, watch tasks, REPLs, database consoles, and similar
  processes. The terminal is a real persistent PTY shown in the Cloudroom UI.
- `list` and `create` require exactly one explicit scope: `--thread <id>`,
  ...
  `ROOM_ENVIRONMENT_ID`, and `ROOM_THREAD_STORAGE`; `cloudroom status` and `--self`
...
- `cloudroom terminal start` is an alias for create. `cloudroom terminal stop` is an alias
  for close.
- Use `cloudroom terminal show`, `attach`, and `resize` for session inspection,
...
- All existing-session operations need only the terminal ID. Use
  `cloudroom terminal wait <terminal-id> --contains "Local:" --timeout 120` to wait
  for readiness from new output. Pass `--from-start` only when matching existing
  scrollback is intentional.
- Use `cloudroom terminal output <terminal-id> --json` to read bounded output, then
  continue with `--since-seq <nextSeq>` when polling. Use
  `cloudroom terminal send <terminal-id> --text "..." --enter` for interactive input,
  `cloudroom terminal rename <terminal-id> <title>` to rename, and
  `cloudroom terminal close <terminal-id>` when the process is no longer needed.
- `cloudroom terminal restart <terminal-id>` replaces the session with a shell in the
  same scope, size, and title. It does not replay the original launch command.

Clearing a thread's parent with `cloudroom thread update --clear-parent-thread` inherits the former parent's section unless the update explicitly supplies a section. Children released by environment archiving also inherit their former parent's section.
