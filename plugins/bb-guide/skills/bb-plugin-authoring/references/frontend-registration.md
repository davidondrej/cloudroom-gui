...
Skills, and plugin panels. Cloudroom keeps the drawer, thread list, footer,
...
Cloudroom without another replacement lookup. Cloudroom restores the original controls if
the selected replacement is unavailable or crashes. Users can select
Automatic, Cloudroom, or one plugin under Settings → Appearance → Navigation.
...
The user can pin Cloudroom's list or a specific provider under
...
interface PluginThreadListProps {
  ...
  searchQuery: string;
  /** Cloudroom's bound thread list. Render it to delegate conditionally without
...
// `indicator` (Cloudroom's resolved status kind) + `indicatorLabel` (its a11y string).
// Draw your own glyph for `indicator`; the SDK ships no status component.
// Treat an unknown indicator value as "none" — cloudroom adds kinds over time.
...
actions.open(id, { split: true }); // Cloudroom's split placement rules
...
actions.requestDelete(id); // opens Cloudroom's delete confirmation
```

Destructive actions deliberately route through the host's own flow, so there
is no silent `delete`: deletion is recursive, and only cloudroom can show the
...
menu from `experimental_useSidebarThreadActions` — it exposes everything Cloudroom's
own menu does, including `requestDelete`, which opens Cloudroom's confirmation.

**Keyboard support is a DOM contract.** Cloudroom's thread shortcuts find rows by
