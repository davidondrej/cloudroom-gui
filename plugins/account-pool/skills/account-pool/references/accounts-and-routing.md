...
browser may be on a different machine from the Cloudroom server. Newly added or
...
`CODEX_POOL_AUTH_TOKEN`; Cloudroom applies them as in-memory app-server config.
...
A Cloudroom server started from inside another Cloudroom server's thread inherits that parent's
