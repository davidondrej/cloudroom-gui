...
- A summary catalog that Room adds to the agent's instructions on every turn. The catalog holds summaries only, up to 3,900 characters.
...
The agent uses the `room memory` CLI. `room memory search` finds summaries with keyword search. `room memory get` reads one complete record. `room memory add`, `room memory update`, and `room memory forget` change the store with version checks. `room memory catalog` and `room memory history` list summaries and past versions.
...
Disable provider-native memory in Settings under Providers to avoid duplicate or conflicting stores. The CLI connects to the local Room server. Some provider sandboxes ask for approval before they allow that connection.
