...
- A summary catalog that Cloudroom adds to the agent's instructions on every turn. The catalog holds summaries only, up to 3,900 characters.
...
Disable provider-native memory in Settings under Providers to avoid duplicate or conflicting stores. The CLI connects to the local Cloudroom server. Some provider sandboxes ask for approval before they allow that connection.
