...
Use `room secret request` when the user needs to supply a credential. Do not ask the user to paste a secret into chat.
...
```bash
room secret request OPENAI_API_KEY RESEND_API_KEY \
