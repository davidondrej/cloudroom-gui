Give an agent the API keys it needs without pasting them into chat. The agent asks, you type the values into a masked form, and Room writes them to a dotenv file on the thread's host.
...
```
room secret request OPENAI_API_KEY --purpose "Configure the server" --describe OPENAI_API_KEY "OpenAI key" --write-env .env.local
```
...
The command must run from a Room thread with a live host.
