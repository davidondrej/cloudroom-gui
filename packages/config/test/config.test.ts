...
describe("common config", () => {
  it("uses the production data dir default in production", () => {
    ...
    ).toBe("/Users/tester/.gui-cloudroom");
  });
  ...
  it("resolves development defaults from the checkout instance", () => {
    ...
    ).toBe("/Users/tester/.gui-cloudroom-dev/src-bb-9039de53a76a");
  });
...
describe("data-dir helpers", () => {
  ...
  it("keeps the legacy fallback label for degenerate checkout labels", () => {
    ...
    ).toBe("/Users/tester/.gui-cloudroom-dev/worktree-41987f975862");
  });
});
...
describe("port helpers", () => {
  ...
  it("rejects whitespace-padded port env values through every port loader path", () => {
    ...
    expect(() =>
      loadCliConfig({
        env: { ROOM_HOST_DAEMON_PORT: " 5555" },
      }),
    ).toThrow("ROOM_HOST_DAEMON_PORT must be a valid TCP port");
  });
});
...
describe("consumer-specific config", () => {
  it("builds server config from explicit runtime env", () => {
    ...
    expect(serverConfig.BB_INFERENCE_FALLBACK).toBe("codex/gpt-5.6-luna");
  ...
  it("requires a valid server URL for the daemon and CLI", () => {
    const env = createHostDaemonRuntimeEnv({
      ...
      ROOM_SERVER_URL: "http://localhost:9999",
    });
    ...
    expect(() =>
      loadCliConfig({ env: { ROOM_SERVER_URL: "not-a-url" } }),
    ).toThrow(/ROOM_SERVER_URL/u);
  });
  ...
  it("defaults CLI connection to Cloudroom even inside official BB", () => {
    ...
    expect(cliConfig.ROOM_SERVER_URL).toBe("http://127.0.0.1:39886");
    expect(cliConfig.ROOM_HOST_DAEMON_PORT).toBe(39887);
  });

  it("lets explicit CLI env overrides win over NODE_ENV-selected defaults", () => {
    const cliConfig = loadCliConfig({
      env: {
        ROOM_HOST_DAEMON_PORT: "3999",
        ROOM_SERVER_URL: "http://localhost:9999",
        BB_SERVER_URL: "http://127.0.0.1:38886",
        BB_HOST_DAEMON_PORT: "38887",
        NODE_ENV: "development",
    ...
    expect(cliConfig.ROOM_SERVER_URL).toBe("http://localhost:9999");
    expect(cliConfig.ROOM_HOST_DAEMON_PORT).toBe(3999);
  });
