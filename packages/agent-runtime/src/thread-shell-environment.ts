...
export function buildThreadShellEnvironment(
  ...
  return {
    ...(args.baseShellEnv ?? {}),
    ...(args.projectId ? { ROOM_PROJECT_ID: args.projectId } : {}),
    ...(args.threadStoragePath
      ? { ROOM_THREAD_STORAGE: args.threadStoragePath }
      : {}),
    ROOM_THREAD_ID: args.threadId,
    ROOM_ENVIRONMENT_ID: args.environmentId,
  };
}
...
export function resolveThreadEnvironment(args: ResolveThreadEnvironmentArgs): {
  ...
  for (const contribution of args.contributedEnv) {
    ...
    if (typeof contribution.value === "string") {
      ...
      const serverUrl = args.baseShellEnv?.ROOM_SERVER_URL;
      if (serverUrl === undefined) {
        entries.push({
          name: contribution.name,
          source: contribution.source,
          value: { masked: true },
          reason: `${contribution.reason} (dropped: no ROOM_SERVER_URL)`,
        });
