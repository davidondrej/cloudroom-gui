...
Spawning:
  ...
  room thread spawn --project <id> --prompt "..." [options]
    ...
    --parent-self                  Parent to the current Local thread (ROOM_THREAD_ID)
    ...
    --machine <id-or-name>         Run on a machine; local selects this Mac, cloud selects the Rust core (--host is an alias)
    --request-id <id>              Stable Cloud creation ID; reuse after an unconfirmed submission
...
Editing a sent message:
  ...
  changes. From an agent thread, the command carries `ROOM_THREAD_ID` so the
...
Messaging:
  ...
  Tell queues Cloud messages automatically. For Local threads it steers into the active
