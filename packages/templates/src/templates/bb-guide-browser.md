...
Cookie import copies signed-in sessions from a browser installed on the desktop host into a Cloudroom browser profile. These two commands take `--host`, `--instance`, and `--generation` but no `--thread`:
