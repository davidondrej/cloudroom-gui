...
describe("run-cli", () => {
  it("runs the source CLI in development mode without a watched build", () => {
    ...
    expect(execution.env.ROOM_SERVER_URL).toBe(expectedDevServerUrl(repoRoot));
    expect(execution.env.ROOM_HOST_DAEMON_PORT).toBe(
      String(expectedDevPorts(repoRoot).hostDaemonPort),
    );
  });

  it("keeps room:dev independent of cli:prepare", async () => {
    const packageJson = await import("../../../package.json", {
      with: { type: "json" },
    });

    expect(packageJson.default.scripts["room:dev"]).not.toContain("cli:prepare");
  });
  ...
  it("lets explicit development CLI targets win", () => {
    vi.stubEnv("NODE_ENV", "development");
    vi.stubEnv("ROOM_SERVER_URL", "http://localhost:4444");
    vi.stubEnv("ROOM_HOST_DAEMON_PORT", "5555");

    const execution = resolveCliExecution(["status"]);

    expect(execution.env.ROOM_SERVER_URL).toBe("http://localhost:4444");
    expect(execution.env.ROOM_HOST_DAEMON_PORT).toBe("5555");
  });
});
