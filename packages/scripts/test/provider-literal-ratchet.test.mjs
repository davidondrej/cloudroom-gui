...
describe("ratchet CLI refusal paths (fixture root)", () => {
  ...
  it.each([".", "gui"])(
    "fails --base when counts rise in workspace %s, even with a regenerated baseline",
    (directory) => {
      const git = (...args) =>
      ...
      const workspace = join(dir, directory);
      write(join(directory, "packages/core/a.ts"), 'const id = "codex";\n');
      write(
        join(directory, "scripts/provider-literal-baseline.json"),
        baseline(
          { "packages/core/a.ts": 1 },
          { "packages/core/a.ts": entry(1) },
        ),
      );
      git("add", "-A");
      git("commit", "-q", "-m", "base");
      ...
      write(
        join(directory, "packages/core/a.ts"),
        'const id = "codex"; const again = "codex";\n',
      );
      write(
        join(directory, "scripts/provider-literal-baseline.json"),
        baseline(
          { "packages/core/a.ts": 2 },
          { "packages/core/a.ts": entry(2) },
        ),
      );
      expect(run([], { BB_RATCHET_ROOT: workspace }).code).toBe(0); // exact match alone passes
      const r = run(["--base", "HEAD"], { BB_RATCHET_ROOT: workspace });
      expect(r.code, r.out).toBe(1);
