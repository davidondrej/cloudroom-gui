...
function createCliEnv(args: CreateCliEnvArgs): NodeJS.ProcessEnv {
  const cliEnv: NodeJS.ProcessEnv = {
    ...
    ROOM_HOST_DAEMON_PORT:
      args.env.ROOM_HOST_DAEMON_PORT ?? String(args.context.daemonPort),
    ROOM_DATA_DIR: args.env.ROOM_DATA_DIR ?? args.context.dataDir,
    NODE_ENV: "production",
  };

  if (cliEnv.ROOM_SERVER_URL === undefined) {
    cliEnv.ROOM_SERVER_URL = args.context.serverUrl;
  }
...
export async function runBbCli(
  ...
  if (env.ROOM_DATA_DIR !== undefined) env.BB_DATA_DIR = env.ROOM_DATA_DIR;
  if (env.ROOM_HOST_DAEMON_PORT !== undefined)
    env.BB_HOST_DAEMON_PORT = env.ROOM_HOST_DAEMON_PORT;
  if (env.ROOM_SERVER_URL !== undefined) env.BB_SERVER_URL = env.ROOM_SERVER_URL;
