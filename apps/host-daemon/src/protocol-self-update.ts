    ...
    async handleProtocolMismatch(
      ...
      try {
        ...
        try {
          ...
          if (response.status === 304 && installedDigest !== null) {
            options.logger.info(
              ...
              "The server-matched Cloudroom host artifact is already installed; restarting the daemon.",
            );
            return "updated";
          }
          if (!response.ok) {
        ...
        options.logger.info(
          ...
          "Installed the server-matched Cloudroom host package; restarting the daemon.",
        );
        return "updated";
      } catch (error) {
        options.logger.error(
