...
describe("EnvironmentSlot", () => {
  ...
  it.each([false, true])(
    "keeps one checkout picker and only shows local inputs when Cloud is selected: %s",
    (selected) => {
      render(
        <EnvironmentSlot
          projectless={false}
          environment={{
            ...
            cloud: { selected, unavailableReason: null, onSelect: vi.fn() },
          }}
          worktree={makeWorktree()}
          environmentProviderInputsSlot={<div>Local branch controls</div>}
        />,
      );
      expect(
        screen.getAllByRole("button", { name: "Environment" }),
      ).toHaveLength(1);
      expect(screen.queryByRole("button", { name: "Local" })).toBeNull();
      expect(screen.queryByRole("button", { name: "Cloud" })).toBeNull();
      expect(Boolean(screen.queryByText("Local branch controls"))).toBe(
        !selected,
      );
      if (selected) {
        expect(
