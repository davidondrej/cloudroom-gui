...
describe("PromptBoxInternal reasoning commands", () => {
  const options: NonNullable<PromptBoxProps["reasoning"]>["options"] = [
    { value: "low", label: "Low" },
  ...
  it.each<[ReasoningLevel, string]>([
    ...
    "selects /%s with %s without sending or losing the draft",
    async (level, key) => {
      const onChange = vi.fn();
      const mention: PromptTextMention = {
        start: 7,
      ...
      const { changes, onSubmit, promptBoxRef } = renderPromptBox(
        `Review @test.ts /${level}`,
        {
          initialMentionRanges: [mention],
          reasoning: { value: "medium", options, onChange },
        },
      );
      await focusPromptEnd(promptBoxRef);
      const label = options.find((option) => option.value === level)!.label;
      const button = await screen.findByRole("button", {
        name: new RegExp(`^${level}\\s*${label} reasoning$`),
      });
      if (key === "click") fireEvent.mouseDown(button, { button: 0 });
      else fireEvent.keyDown(getPromptEditorElement(), { key });

      await waitFor(() => expect(onChange).toHaveBeenCalledWith(level));
      expect(onChange).toHaveBeenCalledTimes(1);
      expect(latestChange(changes)).toEqual({
        value: "Review @test.ts ",
        mentions: [mention],
      });
      await waitForPromptFocus();
  ...
  it.each(["loading", "error"])(
    "works while the skills catalog is %s",
    async (state) => {
      const onChange = vi.fn();
      const { changes, onSubmit, promptBoxRef } = renderPromptBox("/max", {
        reasoning: { value: "medium", options, onChange },
        commandOverrides: {
          trigger: null,
  ...
  it("keeps a same-named skill separate and selectable", async () => {
    const onChange = vi.fn();
    const { changes, promptBoxRef } = renderPromptBox("/max", {
      reasoning: { value: "medium", options, onChange },
      commandSuggestions: [
        {
          kind: "command",
    ...
    fireEvent.keyDown(getPromptEditorElement(), { key: "ArrowDown" });
    fireEvent.keyDown(getPromptEditorElement(), { key: "Enter" });
    await waitFor(() => expect(latestValue(changes)).toBe("/max "));
    expect(latestChange(changes)?.mentions[0]?.resource).toMatchObject({
      kind: "command",
      source: "skill",
      name: "max",
  ...
  it("updates supported levels when the model changes and removes disabled commands", async () => {
    const promptBoxRef = createRef<PromptBoxHandle>();
    const props = createPromptBoxProps({ value: "/", promptBoxRef });
    const onChange = vi.fn();
    const { rerender } = render(
      <PromptBoxInternal
        {...props}
        reasoning={{ value: "max", options: [options[4]!], onChange }}
      />,
    );
    await focusPromptEnd(promptBoxRef);
    await screen.findByRole("button", { name: /^max\s*Max reasoning$/ });
    expect(
      screen.queryByRole("button", { name: /^xhigh\s*Extra High reasoning$/ }),
    ).toBeNull();

    rerender(
      <PromptBoxInternal
        {...props}
        reasoning={{ value: "high", options: [options[2]!], onChange }}
      />,
    );
    await screen.findByRole("button", { name: /^high\s*High reasoning$/ });
    expect(
      screen.queryByRole("button", { name: /^max\s*Max reasoning$/ }),
    ).toBeNull();
