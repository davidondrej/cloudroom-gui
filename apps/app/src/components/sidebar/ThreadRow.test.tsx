...
describe("ThreadRow", () => {
  ...
  it.each(["cloud", "local"] as const)("renders multiline %s titles and fallbacks on one line", (executionTarget) => {
    const title = "Research cloud agents.\n\nRead at least 15 sources.";
    for (const savedTitle of [title, null]) {
      const { container, unmount } = renderThreadRow({
        thread: createThread({ executionTarget, title: savedTitle, titleFallback: title }),
      });
      expect(container.querySelector(".bb-thread-title")?.textContent).toBe("Research cloud agents. Read at least 15 sources.");
      unmount();
    }
  });
