...
describe("SkillsOverview", () => {
  ...
  it("labels the Type filter and preserves independent source toggles", async () => {
    ...
    expect(
      screen.getByLabelText(
        "automations is included with Automations (Cloudroom plugin)",
      ).textContent,
    ).toBe("Included");
  ...
  it("labels the Provider filter and prefixes its logo tooltip", async () => {
    ...
    expect(
      screen.getByRole("menuitemcheckbox", { name: "Cloudroom" }).querySelector("img"),
    ).not.toBeNull();
  });
...
describe("SkillsLibrary library detail routing", () => {
  it("keeps a detail loading state while the skill library resolves", () => {
    ...
    expect(screen.queryByText("New Cloudroom skill")).toBeNull();
  });
  ...
  it.each(["/skills", "/skills/"])(
    ...
    async (path) => {
      ...
      let forkButton = await screen.findByRole("button", {
        name: "Fork Useful skill into a new Cloudroom skill",
      });
  ...
  it("renders the authoritative page order, exposes social proof, and loads more on scroll", () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Fork Alpha into a new Cloudroom skill",
      }).textContent,
    ).toBe("");
    const zuluCreate = screen.getByRole("button", {
      name: "Fork Zulu into a new Cloudroom skill",
    });
...
describe("SkillDetailDialogView", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      accessibleLabel:
        "plugin-notes is included with Skill catalog fixture (Cloudroom plugin)",
