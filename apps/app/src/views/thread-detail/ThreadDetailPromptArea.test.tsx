...
vi.mock("@/components/promptbox/ThreadEnvironmentSummary", () => ({
  ThreadEnvironmentSummary: ({ projectName, environmentLabel, environmentCheckout }: { projectName?: string; environmentLabel?: string; environmentCheckout?: { label: string } }) => (
    <div data-testid="thread-environment-summary">{projectName} · {environmentLabel} · {environmentCheckout?.label}</div>
  ),
}));
...
describe("environment follow-up summary", () => {
  ...
  it.each([
    ...
    if (workspace) testQueryClient.setQueryData(queryKey, workspace);
...
describe("ThreadDetailPromptArea", () => {
  it.each(["pi", "codex"])(
    ...
    (providerId) => {
      ...
      renderPromptArea({
        thread: makeThread({ executionTarget: "cloud", providerId }),
      });
