...
describe("createConnectCredentialCache", () => {
  it("does not access the keychain when no saved credential exists", async () => {
    const encryption = createEncryption();
    const isEncryptionAvailable = vi
      .spyOn(encryption, "isEncryptionAvailable")
      .mockImplementation(() => {
        throw new Error("Unexpected keychain access");
      });
    const decryptString = vi.spyOn(encryption, "decryptString");
  ...
  it("preserves a saved credential when encryption is unavailable", async () => {
    const encrypted = Buffer.from(`sealed:${JSON.stringify(CREDENTIAL)}`);
    const fs = createFs(encrypted);
    ...
    await expect(cache.read()).resolves.toBeNull();
    expect(decryptString).not.toHaveBeenCalled();
    expect(fs.file).toEqual(encrypted);
