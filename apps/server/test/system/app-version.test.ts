...
describe("createAppVersionService", () => {
  it("does not query or recommend upstream updates when disabled for the fork", async () => {
    const calls: FetchCall[] = [];
    const service = createAppVersionService({
      config: { appVersion: "0.0.5", isDevelopment: false },
      fetchImpl: createStubFetch([{ body: { version: "99.0.0" } }], calls),
      logger: testLogger,
      updatesEnabled: false,
    });
    for (const forceRefresh of [false, true]) {
      expect(await service.getSystemVersion({ forceRefresh })).toMatchObject({
