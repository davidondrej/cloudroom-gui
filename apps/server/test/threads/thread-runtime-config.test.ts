  ...
  describe("plugin contributeInstructions assembly", () => {
    ...
    it("appends attributed plugin instructions after tool snippets and before data-dir instructions", async () => {
      await withTestHarness(async (harness) => {
        ...
        const pluginHeader =
          'The following instructions come from the Cloudroom plugin "connect":';
        const dataDirHeader =
          "The following user instructions come from <dataDir>/AGENTS.md:";
    ...
    it("skips null/empty providers, truncates long text, and isolates throws", async () => {
      await withTestHarness(async (harness) => {
        ...
        expect(instructions).not.toContain(
          'The following instructions come from the Cloudroom plugin "nuller":',
        );
        expect(instructions).not.toContain(
          'The following instructions come from the Cloudroom plugin "blank":',
        );
        expect(instructions).not.toContain(
          'The following instructions come from the Cloudroom plugin "boom":',
        );
        expect(instructions).toContain(
          'The following instructions come from the Cloudroom plugin "verbose":',
        );
        expect(instructions).toContain(
          'The following instructions come from the Cloudroom plugin "ok":',
        );
