...
export function resolveServerUrl(context: CliRuntimeContext): string {
  return context.cliConfig.ROOM_SERVER_URL;
}

export function resolveContextProjectId(): string | undefined {
  const fromEnv = toOptionalString(process.env.ROOM_PROJECT_ID);
  if (fromEnv) return validateId(fromEnv, "ROOM_PROJECT_ID");
  return undefined;
}

export function resolveContextThreadId(): string | undefined {
  const fromEnv = toOptionalString(process.env.ROOM_THREAD_ID);
  if (fromEnv) return validateId(fromEnv, "ROOM_THREAD_ID");
  return undefined;
}
...
export function requireThreadIdOrSelf(
  ...
  if (opts.self) {
    ...
    if (!envThreadId) {
      throw new Error("--self requires ROOM_THREAD_ID to be set.");
    }
    return envThreadId;
  }
  if (positionalId) {
...
export function resolveContextSnapshot(
  ...
  return {
    ...
    serverUrl: context.cliConfig.ROOM_SERVER_URL,
  };
}
