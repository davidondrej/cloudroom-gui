...
describe("context-env", () => {
  beforeEach(() => {
    ...
    vi.stubEnv("ROOM_THREAD_ID", undefined);
  });
  ...
  it("does not use ROOM_PROJECT_ID and ROOM_THREAD_ID as explicit ID defaults", () => {
    vi.stubEnv("ROOM_PROJECT_ID", "proj-env");
    vi.stubEnv("ROOM_THREAD_ID", "thread-env");
  ...
  it("normalizes empty values as undefined", () => {
    vi.stubEnv("ROOM_PROJECT_ID", "");
    vi.stubEnv("ROOM_THREAD_ID", "   ");
  ...
  it("resolves connection settings from one CLI runtime context", () => {
    const context = createCliRuntimeContext({
      cliConfig: {
        ROOM_HOST_DAEMON_PORT: 4567,
        ROOM_SERVER_URL: "http://server.test",
      },
    });
  ...
  it("resolves --self from ROOM_THREAD_ID for thread commands", () => {
