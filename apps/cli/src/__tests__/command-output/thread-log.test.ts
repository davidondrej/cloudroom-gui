...
describe("room thread log command output", () => {
  setupCommandOutputTestEnvironment();

  const register: CommandRegistrar = (program) =>
    registerThreadCommands(program, () => "http://server");

  it("room thread log help describes verbose as expanded timeline output", async () => {
  ...
  it("room thread log --json prints raw events", async () => {
    const thread = {
      id: "thread-json-log",
      projectId: "proj-1",
  ...
  it("room thread log renders merged timeline rows for human output", async () => {
  ...
  it("room thread log renders pending steers for human output", async () => {
    const getEvents = vi.fn(async () => []);
    const getTimeline = vi.fn(async () =>
  ...
  it("room thread log renders pending steers with default formatting", async () => {
    const getEvents = vi.fn(async () => []);
    const getTimeline = vi.fn(async () =>
      fixtures.makeTimelineResponse([fixtures.makePendingSteerTimelineRow()]),
    );
  ...
  it("room thread log renders approval state on command and file-change rows", async () => {
    const getEvents = vi.fn(async () => []);
    const getTimeline = vi.fn(async () =>
      fixtures.makeTimelineResponse([
        {
  ...
  it("room thread log --json caps at --limit and warns on stderr when more events exist", async () => {
  ...
  it("room thread log --json detects more events across the server page boundary", async () => {
  ...
  it("room thread log --json retries smaller raw-event pages after the byte limit", async () => {
  ...
  it("room thread log --json stays quiet when the page is not full", async () => {
    const events = [
      {
        id: "evt-1",
        scope: { kind: "thread" },
  ...
  it("room thread log --json --all pages through every event with --after-seq", async () => {
  ...
  it("room thread log --json --all keeps paging after reducing a byte-heavy page", async () => {
    const events = Array.from({ length: 5 }, (_, index) => ({
      id: `evt-${index + 1}`,
      scope: { kind: "thread" },
      threadId: "thread-json-log",
  ...
  it("room thread log --json --all restores large pages after a dense prefix", async () => {
  ...
  it("room thread log prints an older-history notice when the timeline page is cut", async () => {
    const getTimeline = vi.fn(async () => ({
      ...fixtures.makeTimelineResponse([
  ...
  it("room thread log --limit sets the timeline segment limit for human output", async () => {
    const getTimeline = vi.fn(async () =>
      fixtures.makeTimelineResponse([fixtures.makePendingSteerTimelineRow()]),
    );
    stubServerApi({
  ...
  it("room thread log --all walks older timeline pages and prints them oldest first", async () => {
  ...
  it("room thread log rejects --all combined with --limit", async () => {
    stubServerApi({
  ...
  it("room thread log --self resolves from ROOM_THREAD_ID", async () => {
