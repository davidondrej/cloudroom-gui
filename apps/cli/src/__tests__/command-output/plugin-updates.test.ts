...
describe("cloudroom plugin update commands", () => {
  ...
  it("outdated renders every outcome, reasons, and the dev-build annotation", async () => {
      ...
      jsonResponse({
        results: [
          ...
          },
          {
            ...
            devMode: true,
            installed: version("1.0.0"),
            blocked: { version: "2.0.0", reasons: ["requires room >= 9"] },
          },
          {
            id: "e",
            outcome: "unavailable",
    ...
    expect(output).toContain("2.0.0: requires room >= 9");
  ...
  it("--all updates compatible results and skips pinned and incompatible", async () => {
        ...
        jsonResponse({
          results: [
            { id: "pin", outcome: "pinned", installed: version("1") },
            {
              ...
              outcome: "incompatible",
              installed: version("1"),
              blocked: { version: "2", reasons: ["requires newer room"] },
            },
            {
              id: "good",
              outcome: "update-available",
    ...
    expect(output).toContain("bad: skipped — incompatible: requires newer room");
