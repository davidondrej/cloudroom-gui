...
describe("room thread open command output", () => {
  ...
  it("uses ROOM_THREAD_ID and opens a thread-relative workspace path", async () => {
    vi.stubEnv("ROOM_THREAD_ID", "thread-current");
  ...
  it("uses an explicit thread id when ROOM_THREAD_ID is not set", async () => {
    const { openThread } = stubThreadOpenApi({
  ...
  it("resolves an absolute workspace path through the target thread environment", async () => {
    vi.stubEnv("ROOM_THREAD_ID", "thread-workspace");
  ...
  it("opens an absolute thread-storage path for the current thread", async () => {
    vi.stubEnv("ROOM_THREAD_ID", "thread-storage");
    vi.stubEnv("ROOM_THREAD_STORAGE", "/tmp/bb-thread-storage/thread-storage");
    const { getEnvironment, getThread, openThread } = stubThreadOpenApi({
      open: async () => ({ delivered: 1 }),
    });
  ...
  it("rejects a different explicit thread id when ROOM_THREAD_ID is set", async () => {
    vi.stubEnv("ROOM_THREAD_ID", "thread-current");
    stubThreadOpenApi({});
