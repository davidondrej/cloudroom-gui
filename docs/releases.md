...
- Building is account-free. The GUI currently requires an invite-enabled Cloudroom account, even for local work ([join the waitlist](https://www.cloudroom.dev/#waitlist)). Manual self-hosted core configuration does not bypass that gate. The core remains independently usable without the GUI.
